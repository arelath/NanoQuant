...
class RuntimeBenchmarkRequest:
    """Typed, reusable packed-runtime benchmark protocol and material inputs."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.max_new_tokens <= 0 or self.stopping_check_interval <= 0:
            raise ValueError("runtime benchmark generation lengths must be positive")
...
def _bound_tensor_operation(
    operation: Callable[[torch.Tensor], object],
    value: torch.Tensor,
) -> Callable[[object | None], object]:
    def run(_state: object | None) -> object:
        return operation(value)

    return run


def _bound_prepared_linear(
    module: PreparedLinear,
    workload: WorkloadKind,
    value: torch.Tensor,
) -> Callable[[object | None], object]:
    def run(_state: object | None) -> object:
        with execution_workload(workload):
            return module(value)
...
def _bound_generation(
    operation: Callable[[GenerationRequest], object],
...
def run_runtime_benchmark(request: RuntimeBenchmarkRequest) -> dict[str, Any]:
    """Execute a validated benchmark request and return its structured result."""

    return _benchmark(request)


@torch.inference_mode()
def _benchmark(args: RuntimeBenchmarkRequest) -> dict[str, Any]:
    ...
    with wait_for_device_lease(str(device), args.wait_for_device_seconds):
            ...
            if "kernel" in selected_suites:
                cases.append(
                    _benchmark_cuda_case(
                        ...
                        _bound_tensor_operation(plan.dispatches[layer_index].linear, value),
                        setup=None,
                        args=args,
                        device=device,
            ...
            if "layer" in selected_suites:
                ...
                cases.append(
                    _benchmark_cuda_case(
                        "layer",
                        f"{workload}:{layer_name}",
                        _bound_prepared_linear(module, cast(WorkloadKind, workload), value),
                        setup=None,
                        args=args,
                        device=device,
                        unit_name="token_slots",
        ...
        def prefill(_state: object | None = None) -> GenerationStep:
        ...
        if "end-to-end" in selected_suites:
            ...
            for name, generation_request, units in (
                ...
                distribution = benchmark_wall(
                    _bound_generation(run_generation, generation_request),
                    warmups=args.warmups,
                    repetitions=args.repetitions,
...
def main() -> None:
    ...
    result = run_runtime_benchmark(request)
    if args.output is not None:
        atomic_write_json(args.output, result)
    print(json.dumps(result, sort_keys=True, indent=2))


__all__ = ["RuntimeBenchmarkRequest", "run_runtime_benchmark"]
