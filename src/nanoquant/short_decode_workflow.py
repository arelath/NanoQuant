"""Numbered-experiment composition for the paired short-decode benchmark."""
...
class ShortDecodeBenchmarkExperiment:
...
def resolve_short_decode_experiment(
    config: RunConfig,
    experiment: ShortDecodeBenchmarkExperiment,
    *,
    ...
    return ShortDecodeBenchmarkExperiment(
        replace(
            request,
            snapshot=snapshot,
            ...
            runtime_bundle=_resolve(request.runtime_bundle, repository_root),
        ),
...
def execute_short_decode_experiment(
    config: RunConfig,
    experiment: ShortDecodeBenchmarkExperiment,
    *,
    launcher_path: str | Path,
    ...
    resolved = resolve_short_decode_experiment(config, experiment, launcher_path=launcher_path)
    result = execute_short_decode_benchmark(resolved.request)
    payload = {
        **result,
        "experiment": {
...
def run_short_decode_experiment(
    config: RunConfig,
    experiment: ShortDecodeBenchmarkExperiment,
    *,
    launcher_path: str | Path,
) -> int:
    execute_short_decode_experiment(config, experiment, launcher_path=launcher_path)
    return 0
