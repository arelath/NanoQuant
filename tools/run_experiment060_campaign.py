"""Run Experiment 060 in clean one-block process slices until quality completion."""
...
_RUN_NAME = "060-best-methods-2bpw-compress-and-benchmark-gemma-3-1b-it"
_CONTROL_NAME = "060-d2-uniform-control-gemma-3-1b-it"
...
def main() -> int:
    ...
    campaign_root = repository / "evidence" / "060"
    ...
    if first_slice > args.maximum_slices:
        raise RuntimeError(
            f"Experiment 060 already used {first_slice - 1} of {args.maximum_slices} allowed slices"
        )
    for index in range(first_slice, args.maximum_slices + 1):
        ...
        print(f"starting Experiment 060 slice {index}; durable block commits={before}", flush=True)
        ...
        if completed.returncode == 0:
            print(
                f"Experiment 060 completed in slice {index}; durable block commits={after}",
                flush=True,
            )
            return 0
    ...
    raise RuntimeError("Experiment 060 exceeded its block-bounded slice limit")
