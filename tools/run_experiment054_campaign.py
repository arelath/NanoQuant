...
def _completed_blocks(campaign_root: Path) -> int:
    completed = 0
    for journal in campaign_root.glob("*/state/journal.jsonl"):
...
def main() -> int:
    ...
    launcher = (
        repository
        / "experiments"
        / "054-functional-binary-lr-d2-compress-and-benchmark-gemma-3-1b-it.py"
    )
    ...
    for index in range(first_slice, args.maximum_slices + 1):
        before = _completed_blocks(campaign_root)
        ...
        after = _completed_blocks(campaign_root)
