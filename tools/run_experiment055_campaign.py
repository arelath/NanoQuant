"""Run Experiment 055 in clean one-block process slices until completion."""
...
def main() -> int:
    ...
    if first_slice > args.maximum_slices:
        raise RuntimeError(
            f"Experiment 055 already used {first_slice - 1} of {args.maximum_slices} allowed slices"
        )
    for index in range(first_slice, args.maximum_slices + 1):
        ...
        print(f"starting Experiment 055 slice {index}; completed blocks={before}", flush=True)
        ...
        if completed.returncode == 0:
            print(f"Experiment 055 completed in slice {index}; completed blocks={after}", flush=True)
            return 0
    ...
    raise RuntimeError("Experiment 055 exceeded its block-bounded slice limit")
