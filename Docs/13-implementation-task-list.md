...
- [x] **M4.29** Run the pinned 1B eight-epoch model-level KD protocol, evaluate the tuned artifact, and compare its quality/memory/time with the retained legacy result. The versioned legacy Python/device-RNG cache sampler completed 2,048 steps at 2.70 GB peak allocated CUDA memory and reached exact serial PPL 454.431 versus immutable pre-KD 432.931 and the retained legacy checkpoint through the rewrite backend at 383.939. This completes the experiment but fails the quality gate; the remaining gap is upstream frozen factor/outlier state, not the KD execution boundary.
